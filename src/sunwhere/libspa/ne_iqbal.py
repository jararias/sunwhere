...
import numpy as np
from numexpr import evaluate as nex
...
def eccentricity_correction_factor(dayangl):
    ...
    return nex(
        '1.00012825e+00'
        '+ 3.33572253e-02*cos(  dayangl) + 1.99417932e-03*sin(  dayangl)'
        '+ 6.97434604e-04*cos(2*dayangl) + 1.14828177e-04*sin(2*dayangl)'
        '+ 1.74028675e-05*cos(3*dayangl) + 1.13778023e-05*sin(3*dayangl)')
...
def equation_of_time(dayangl):
    ...
    return nex(
        '(0.00986571'
        ' + 0.58688718*cos(  dayangl) - 7.34538133*sin(  dayangl)'
        ' - 3.31493999*cos(2*dayangl) - 9.35366541*sin(2*dayangl)'
        ' - 0.08151750*cos(3*dayangl) - 0.30892409*sin(3*dayangl)'
        ' - 0.13532889*cos(4*dayangl) - 0.17336220*sin(4*dayangl))/60.')
...
def solar_declination(dayangl):
    ...
    return nex(
        '0.006918'
        '- 0.399912*cos(  dayangl) + 0.070257*sin(  dayangl)'
        '- 0.006758*cos(2*dayangl) + 0.000907*sin(2*dayangl)'
        '- 0.002697*cos(3*dayangl) + 0.001480*sin(3*dayangl)')
...
def _sunpos_one_dimensional(unixtime, longitude, latitude, with_refraction):
    ...
    sinDecl = nex('sin(declination)')
    cosDecl = nex('cos(declination)')

    lat_rad = latitude*RAD
    cosLat = nex('cos(lat_rad)')
    sinLat = nex('sin(lat_rad)')

    LAT = nex('std_time + longitude/15. + eot')  # hours, Eq (1.4.2)
    LAT = np.where(nex('LAT < 0'), LAT + 24., LAT)
    LAT = np.where(nex('LAT >= 24'), LAT - 24., LAT)
    hour_angle = nex('(LAT - 12.)*15.*RAD')
    cosHour = nex('cos(hour_angle)')

    sza_rad = nex('arccos(sinDecl*sinLat + cosDecl*cosLat*cosHour)')
    ...
    azi_rad = nex('arctan2(-sinHour, tanDecl*cosLat - sinLat*cosHour)')
    azi_rad = np.where(nex('azi_rad < 0'), azi_rad + PI, azi_rad - PI)
...
def _sunpos_two_dimensional(unixtime, longitude, latitude, with_refraction):
    ...
    sinDecl = nex('sin(declination)')[:, None]
    cosDecl = nex('cos(declination)')[:, None]
...
def _sunpos_three_dimensional(unixtime, longitude, latitude, with_refraction):
    ...
    sinDecl = nex('sin(declination)')[:, None, None]
    cosDecl = nex('cos(declination)')[:, None, None]

    lat_rad = latitude[None, :, None]*RAD
    cosLat = nex('cos(lat_rad)')
    sinLat = nex('sin(lat_rad)')

    LAT = (std_time[:, None, None]
            + longitude[None, None, :]/15.
